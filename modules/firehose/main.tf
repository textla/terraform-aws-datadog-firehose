...
resource "aws_kinesis_firehose_delivery_stream" "kinesis_firehose_stream_logs" {
  ...
  http_endpoint_configuration {
    ...
    s3_backup_mode     = var.backup_all_data_to_s3 ? "AllData" : "FailedDataOnly"

    s3_configuration {
      role_arn            = var.role_arn
      bucket_arn          = var.backup_all_data_to_s3 ? aws_s3_bucket.all_data[0].arn : aws_s3_bucket.failed[0].arn
      prefix              = var.backup_all_data_to_s3 ? "successful/" : null
      error_output_prefix = var.backup_all_data_to_s3 ? "failed/" : null
    }
...
resource "aws_kms_key" "failed" {
  count               = var.backup_all_data_to_s3 ? 0 : 1
...
# All logs S3 bucket (when backup_all_data_to_s3 is enabled)
resource "aws_kms_key" "all_data" {
  count               = var.backup_all_data_to_s3 ? 1 : 0
  description         = "${var.name}-all-data"
  enable_key_rotation = true
  policy              = data.aws_iam_policy_document.all_data[0].json
}

data "aws_iam_policy_document" "all_data" {
  count = var.backup_all_data_to_s3 ? 1 : 0
  statement {
    sid = "CMKOwnerPolicy"
...
resource "aws_s3_bucket" "all_data" {
  count  = var.backup_all_data_to_s3 ? 1 : 0
  bucket = "${data.aws_caller_identity.current.account_id}-${var.name}"
}

resource "aws_s3_bucket_lifecycle_configuration" "all_data" {
  count  = var.backup_all_data_to_s3 ? 1 : 0
  bucket = aws_s3_bucket.all_data[0].id
...
resource "aws_s3_bucket_logging" "all_data" {
  count         = var.backup_all_data_to_s3 ? 1 : 0
  bucket        = aws_s3_bucket.all_data[0].id
  target_bucket = var.s3_access_log_bucket
  target_prefix = "all-data-access/"
}

resource "aws_s3_bucket_server_side_encryption_configuration" "all_data" {
  count  = var.backup_all_data_to_s3 ? 1 : 0
  bucket = aws_s3_bucket.all_data[0].bucket
  rule {
    apply_server_side_encryption_by_default {
      kms_master_key_id = aws_kms_key.all_data[0].arn
      sse_algorithm     = "aws:kms"
    }
  }
}

resource "aws_s3_bucket_public_access_block" "all_data" {
  count  = var.backup_all_data_to_s3 ? 1 : 0
  bucket = aws_s3_bucket.all_data[0].id
...
resource "aws_s3_bucket_versioning" "all_data" {
  count  = var.backup_all_data_to_s3 ? 1 : 0
  bucket = aws_s3_bucket.all_data[0].id
  versioning_configuration {
    status = "Enabled"
